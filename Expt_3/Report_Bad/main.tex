...
\end{figure}
An operational amplifier can be operated as an inverting amplifier as shown in the figure. Biasing supply to op-amp, $ V_{S \pm} $  = $\pm 15$ V must be applied. As the op-amp has very high impdedence, we can write the equation as,
\begin{equation*}
	\frac{V_{in}- v_a}{R_i} = \frac{V_a - V_{out}}{R_f}.
\end{equation*}
And $ V_a $ is the virtual ground, so $ V_a = 0 $, so the voltage gain in inverting op-amp is,
\begin{equation*}
...
\end{figure}
An operational amplifier can be operated as a non-inverting amplifier as shown in the figure. 
From the circuit diagram, we can write that 
\begin{equation*}
	\frac{0 - V_a}{R_i} = \frac{V_a - V_{out}}{R_f}.
\end{equation*}
As, $ V_a = V_{in} $ the gain for this circuit is, 
\begin{equation*}
...
\end{figure}
In the above configuration, op-amp circuit acts as a subtractor circuit. It subtracts one signal from the other signals.
\begin{equation*}
 \frac{V_1 - V_a}{R_1} = \frac{V_a - V_{out}}{R_f} 	.
\end{equation*}
\begin{equation*}
